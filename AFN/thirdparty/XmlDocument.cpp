...
XmlDocument::XmlDocument(const char* Stream,int StreamSize,const std::string& verify_ns,const std::string& verify_root)
: m_doc(NULL)
, m_ok(false)
{
	xmlNodePtr cur;
	...
	if (!(m_doc = xmlParseMemory(Stream,StreamSize)))
	{
		throw XmlException("Parse of Stream failed");
	}
	if (!(cur = xmlDocGetRootElement(m_doc)))
	{
		xmlFreeDoc(m_doc);
		m_doc = NULL;
		throw XmlException("Document from Stream is empty");
	}
	if (verify_ns.size())
	{
		if (!(ns = xmlSearchNsByHref(m_doc, cur, (const xmlChar *) verify_ns.c_str() )))
